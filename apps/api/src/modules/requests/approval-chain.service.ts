...
import { Prisma, ApprovalChainStatus, ApprovalChainScope, ApproverType, ApprovalMode, ConflictResolution } from '@prisma/client';
...
export async function createApprovalChain(
  ...
  if (existing) {
    throw new ApiError('Approval chain code already exists', 409, 'DUPLICATE_CODE');
  }
  ...
  logger.info(`Approval chain created: ${chain.code}`, { chainId: chain.id, tenantId });
